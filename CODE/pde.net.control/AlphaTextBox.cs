...
namespace pde.net.control
{
    ...
    public class AlphaTextBox : TextBox
    {
        ...
        private void UpdateRegion()
        {
            InternalBGSet = true;
            Color tmp = BackColor;
            if (tmp.Equals(Color.Transparent))
            {
                BackColor = this.Parent==null? Color.White : this.Parent.BackColor;
            }
            if (ClientRegionBitmap != null)
                ClientRegionBitmap.Dispose();

            ClientRegionBitmap = TBUtils.MapColors(BackColor, Color.FromArgb(AlphaBackColorAmount, BackColor), CaptureClientRegion(), true);
        ...
        private void DrawCaretToBitmap()
        {
            ...
            if (DrawCaret)
            {
                g.FillRectangle(new SolidBrush(this.ForeColor), CaretPosition.X, CaretPosition.Y, this.Font.SizeInPoints / 5, this.FontHeight);
            }
         ...
         public override Color BackColor
       {
        ...
        public int AlphaBackColorAmount
        {
            get
            {
        ...
        [Category("Appearance"), Description("The visible background color for the AlphaTextBox."), Browsable(false),
        DesignerSerializationVisibility(DesignerSerializationVisibility.Visible)]
